    ...
    public class DatabaseInitializer
    {
        public static async Task InitializeAsync()
        {
            try
            {
                ...
                // ГАРАНТИРОВАННО ЗАПОЛНЯЕМ ДАННЫМИ
                ...
                await context.SaveChangesAsync();
                Console.WriteLine($"Created {realGifts.Count} EGO gifts!");

                // СОЗДАЕМ ИСТОЧНИКИ (они уже созданы вместе с дарами через навигационные свойства)
                var giftsCount = await context.EgoGifts.CountAsync();
                var sourcesCount = await context.Sources.CountAsync();
                Console.WriteLine($"Database contains: {giftsCount} gifts, {sourcesCount} sources");
